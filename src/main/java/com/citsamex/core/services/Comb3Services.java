 ...
 *
 */
public class Comb3Services extends SuperServices{
...
//				JOptionPane.showMessageDialog(this, "不存在页签");
	...
	public List<VoucherEntryVO> convertFile2VO(ArrayList<String[]> list,MainStart mainUI) throws Exception{
		...
		for(String[] str : list){
			...
			//检查第一个辅助核算项	销售商			
			String sql = "select FItemID from t_item where fname like '%" + str[0] + "' and FItemClassID=8";
			voucherEntryvo.setFxss(getFItemID(sql, str[0]));
			//检查第二个辅助核算项	基金代码
			sql = "select FItemID from t_item where fnumber like '%" + str[1] + "' and FItemClassID=2039";
